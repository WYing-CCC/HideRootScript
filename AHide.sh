...
# URL定义
MODULES_URL="http://github.moeyy.xyz/https://github.com/WYing-CCC/WYing-HRPS/releases/download/Download/installmodule.zip"
JZMK_URL="http://github.moeyy.xyz/https://github.com/WYing-CCC/WYing-HRPS/releases/download/Download/jzmk.zip"
CONFIG_URL="http://github.moeyy.xyz/https://github.com/WYing-CCC/WYing-HRPS/releases/download/Download/config.zip"
APK_URL="http://github.moeyy.xyz/https://github.com/WYing-CCC/WYing-HRPS/releases/download/Download/installapk.zip"
APCONFIG_URL="http://github.moeyy.xyz/https://github.com/WYing-CCC/WYing-HRPS/releases/download/Download/package_config"
HIDE_URL="http://github.moeyy.xyz/https://github.com/WYing-CCC/WYing-HRPS/releases/download/Download/hide.sh"
MOMO_URL="http://github.moeyy.xyz/https://github.com/WYing-CCC/WYing-HRPS/releases/download/Download/momo.sh"
Loop_URL="http://github.moeyy.xyz/https://github.com/WYing-CCC/WYing-HRPS/releases/download/Download/000.zip"
KEYBOX_URL="http://github.moeyy.xyz/https://github.com/WYing-CCC/WYing-HRPS/releases/download/Download/keybox.sh"
ART_URL="http://github.moeyy.xyz/https://github.com/WYing-CCC/WYing-HRPS/releases/download/Download/ART.zip"
INIT_RC_URL="http://github.moeyy.xyz/https://github.com/WYing-CCC/WYing-HRPS/releases/download/Download/Init_rc.zip"
SELINUX_URL="http://github.moeyy.xyz/https://github.com/WYing-CCC/WYing-HRPS/releases/download/Download/Selinux.zip"
DNP_URL="http://github.moeyy.xyz/https://github.com/WYing-CCC/WYing-HRPS/releases/download/Download/DNP.zip"
# 创建目录
mkdir -p $YC_DIR
mkdir -p $MODULE_DIR
mkdir -p "/data/HRPS"
...
download_file() {
    ...
    while [ $retry_count -lt $max_retry ]; do
        ...
        if command -v aria2c >/dev/null; then
            ...
            curl -L --connect-timeout $timeout -A "$user_agent" -o "$output" "$url" && break
...
PIF_URL="http://github.moeyy.xyz/https://github.com/WYing-CCC/WYing-HRPS/releases/download/Download/PlayIntegrityFix.zip"
LSP_DIR="$YC_DIR/LSP.zip"
LSP_URL="http://github.moeyy.xyz/https://github.com/WYing-CCC/WYing-HRPS/releases/download/Download/LSP.zip"
ZN_DIR="$YC_DIR/ZygiskNext.zip"
ZN_URL="http://github.moeyy.xyz/https://github.com/WYing-CCC/WYing-HRPS/releases/download/Download/ZygiskNext.zip"
echo "过Native Test环境"
echo "[${GREEN}f${NC}] ${YELLOW}返回主界面${NC}"
...
Test_Root() {
    ...
    # 检查APatch
    if [ -d "/data/adb/apd" ] || [ -d "/data/user/0/me.bmax.apatch/" ]; then
        echo "${YELLOW}你的Root管理器为：${NC}APatch $SU_VERSION_NUM"
        return
    fi

    
    # 检查APatchNext
    if [ -d "/data/adb/apd" ] || [ -d "/data/user/0/me.garfieldhan.apatch.next/" ]; then
        echo "${YELLOW}你的Root管理器为：${NC}APatchNext $SU_VERSION_NUM"
        return
    fi
    
    # 检查KernelSU
    if [ -d "/data/adb/ksud" ] || [ -d "/data/user/0/me.weishu.kernelsu/" ]; then
        echo "${YELLOW}你的Root管理器为：${NC}KernelSU $SU_VERSION_NUM"
        return
    fi

    # 检查KernelSUNext
    if [ -d "/data/adb/ksud" ] || [ -d "/data/user/0/com.rifsxd.ksunext/" ]; then
    ...
    if echo "$SU_VERSION" | grep -qi "alpha"; then
        ...
        echo "${YELLOW}未检测到Magisk或兼容的Root管理器${NC}"
    fi
}
...
echo -e "${NC}"
    ...
    sleep 0.025
    echo -e "[${GREEN}D${NC}] ${YELLOW}开发者${NC}"
    sleep 0.025
...
EOF
    ...
    echo -e "${GREEN}—————————————————————————————————————————————————————${NC}"
    echo -e "${TRUE_ORANGE}当前版本：AHideV1开源${NC}"
    echo -e ""
